      ...
      case Action of
        cnAdded:
          begin
            aGrid:=FECGrid;
            aGrid.Flags:=aGrid.Flags+cHorFlags;
            if aGrid.UpdateCount=0 then  { ~3% faster than InvalidateNonUpdated; }
              begin
                aGrid.DoChanges;
                aGrid.Invalidate;
              end;
...
begin
  Flags:=Flags+cRedrawGrid;
  if IsEnabled
    then include(CommonPaintFlags, pfEnabled)
    else exclude(CommonPaintFlags, pfEnabled);
    ...
    begin
      ...
      if Key in [VK_PRIOR..VK_DOWN]
        then Key:=0
          ...
          if bSelectEd and not EditorMode and not (egoReadOnly in Options) and
...
begin
  ...
  while assigned(aColumnNode) do  { DeleteCol old children }
    begin
      aNode:=aColumnNode;
